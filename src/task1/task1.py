...
class Predictor:
    ...
    def preprocess_data(self, df_raw: DF) -> DF:
        ...
        df_preprocessed = pd.get_dummies(
            df_preprocessed, columns=["electricity_last_contract_term", "province"], drop_first=True
        )
    ...
    def process_predictions(self, hashed_ids: np1d, buy_toon_chance: np1d) -> DF:
        ...
        df_output = pd.concat(
            [customer_id, prospects.reset_index(drop=True), chances.reset_index(drop=True)], axis=1
    ...
    def process_dataset(self, data: Union[DF, str]) -> DF:
        ...
        self.processing_time = datetime.now().strftime(
            "%Y%m%d_%H_%M_%S"
        )  # NB: will overwrite if processing < 1 sec
        ...
        if df_preprocessed.isna().sum().sum() != 0:
            ...
            logger_task1.error(
                "Preprocessing is not done properly, there are NaN values in the dataset!"
            )
            raise ValueError(
                "Preprocessing is not done properly, there are NaN values in the dataset!"
            )
        ...
        df_output = self.process_predictions(
            hashed_ids,
            buy_toon_chance,
        )
